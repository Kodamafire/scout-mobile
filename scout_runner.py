...
def submit_upgrade_sell_if_allowed(trading, upgrade, market_open, open_orders):
    ...
    if not CFG.upgrade_execute_trades:
        return "WATCH ONLY — UPGRADE IDENTIFIED"
    if not market_open:
...
def run_scout_cycle():
    ...
    upgrade = choose_upgrade(managed, candidates, held, open_orders)
    if upgrade:
    ...
    if upgrade:
        print(
            ...
            f"weak exit score {upgrade['replace_exit_score']} | {upgrade['order_status']}"
        )
    if managed:
