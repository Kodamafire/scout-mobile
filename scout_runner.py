...
class ScoutConfig:
    ...
    upgrade_execute_trades: bool = False
...
def submit_upgrade_sell_if_allowed(trading, upgrade, market_open, open_orders):
    """Upgrade execution has its own safety switch and is WATCH ONLY by default."""
    if not upgrade:
        return None
    if not upgrade.get("upgrade_confirmed", False):
        return (
            f"WATCH ONLY — CONFIRMING "
            f"{upgrade.get('upgrade_confirmation', 0)}/"
            f"{CFG.upgrade_confirmations_required}"
        )
    if not CFG.upgrade_execute_trades:
        return "WATCH ONLY — UPGRADE CONFIRMED"
    if not market_open:
        return "WATCH ONLY — MARKET CLOSED"
    symbol = upgrade["replace_symbol"]
    if symbol in open_orders:
        return "SKIPPED — OPEN ORDER EXISTS"
    qty = abs(float(upgrade["replace_qty"]))
    if qty <= 0:
        return "SKIPPED — NO QUANTITY"
    order = trading.submit_order(order_data=MarketOrderRequest(
        symbol=symbol, qty=qty, side=OrderSide.SELL, time_in_force=TimeInForce.DAY
    ))
    return f"UPGRADE SELL SUBMITTED {getattr(order, 'id', '')}"
...
def run_scout_cycle():
    ...
    print("=" * 70)
    print("SCOUT TRADER V46 AUTOMATIC — PAPER CONTROL CENTER")
    print("Cycle:", cycle)
    print("=" * 70)
    ...
    if upgrade:
        upgrade["order_status"] = submit_upgrade_sell_if_allowed(
    ...
    report = {
        "version": 46, "cycle": cycle, "updated": started.strftime("%b %d, %Y %I:%M:%S %p PT"),
