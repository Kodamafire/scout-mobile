...
def mount_and_load_state():
    ...
    default = {"version": 44, "high_water": {}, "warning_streak": {}, "last_cycle": None}
...
def submit_buys_if_allowed(trading, candidates, account, market_open, held_symbols, open_orders):
    ...
    budget_each = min(equity * CFG.position_fraction, spendable / max(1, limit)) if limit else 0
    for row in candidates[:limit]:
        ...
        results.append({**row, "order_status": status})
    return results
...
def run_scout_cycle():
    ...
    print("=" * 70)
    print("SCOUT TRADER V44 AUTOMATIC — PAPER CONTROL CENTER")
    print("Cycle:", cycle)
    print("=" * 70)
    ...
    report = {
        "version": 44, "cycle": cycle, "updated": started.strftime("%b %d, %Y %I:%M:%S %p PT"),
    ...
    print(f"Positions analyzed: {len(managed)} | Qualified candidates: {len(buy_results)} | Dashboard: {publish_status}")
    if managed:
